...
def test_custom_distance_calculator():
    ...
    result = blocklist_validator("ComplexPass", ["Complete"], {"customDistanceCalculator": custom_distance_calculator})
...
def test_reject_exact_blocklist_terms():
    cases = [
        {"password": "mypassword123", "blocklist": ["password"], "expected_error": "password"},
        {"password": "secure_pass", "blocklist": ["secure_pass"], "expected_error": "secure_pass"},
        {"password": "mypassword😊", "blocklist": ["password😊"], "expected_error": "password😊"}
    ]
    for case in cases:
        result = blocklist_validator(case["password"], case["blocklist"])
        assert result["isValid"] is False
        # Strip both strings before comparison to remove extra spaces
        assert f'Password contains a substring too similar to: "{case["expected_error"]}".'.strip() in [error.strip() for error in result["errors"]]
...
def test_fuzzy_matching():
    ...
    for case in cases:
        result = blocklist_validator(case["password"], case["blocklist"], {"matchingSensitivity": 0.5})
        assert result["isValid"] is False
...
def test_whitespace_handling():
    ...
    trimmed_result = blocklist_validator(password, [blocklist_term], {"trimWhitespace": True})
    assert trimmed_result["isValid"] is False

    # With trimWhitespace disabled
    untrimmed_result = blocklist_validator(password, [blocklist_term], {"trimWhitespace": False})
...
def test_use_fuzzy_for_long_terms():
    password = "testlongerterm"
    long_term = "longerterm"

    # Mock Levenshtein to ensure it is called for longer terms
    with patch("src.blocklist_validator.levenshtein_distance") as mock_levenshtein:
        result = blocklist_validator(password, [long_term], {"matchingSensitivity": 0.25})
        # Check if the Levenshtein distance function was called
        mock_levenshtein.assert_called_once_with(password, long_term)
